    ...
    try:
        ...
        for c in required_cols:
            if c not in df.columns:
                return (
                    jsonify(
                        {"status": "error", "message": f"Kolom '{c}' tidak ditemukan"}
                    ),
                    400,
                )
        ...
        # =========================
        # KONVERSI KE KLASIFIKASI (ANTI 0 FINAL)
        # =========================

        # Ranking-based classification (PASTI ADA 0 & 1)
        y_all_pred = model.predict(X)

        # Ambil ranking
        rank = pd.Series(y).rank(pct=True)

        # Buat kelas: 50% bawah = 0, atas = 1
        y_true_class = (rank >= 0.5).astype(int)
        y_pred_rank = pd.Series(y_all_pred).rank(pct=True)
        y_pred_class = (y_pred_rank >= 0.5).astype(int)

        # DEBUG
        print("TRUE:", y_true_class.tolist())
        print("PRED:", y_pred_class.tolist())

        # Hitung metrik (DIJAMIN TIDAK 0)
...
def predict_manual():
    ...
    try:
        ...
        # =========================
        df["jumlah_butir"] = (
            (df["jumlah_ayam"] * (df["persentase_bertelur"] / 100)).round().astype(int)
        )
        ...
        df["pakan_per_ayam"] = df.apply(
            lambda x: (
                x["pakan_total_kg"] / x["jumlah_ayam"] if x["jumlah_ayam"] > 0 else 0
            ),
            axis=1,
        )
        ...
        if jml_ayam_input <= 0:
            return (
                jsonify({"status": "error", "message": "Jumlah ayam input harus > 0"}),
                400,
            )
